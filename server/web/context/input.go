...
// RungoInput operates the http request header, data, cookie and body.
...
// NewInput returns the RungoInput generated by context.
func NewInput() *RungoInput {
	return &RungoInput{
...
// var id int  RungoInput.Bind(&id, "id")  id ==123
// var isok bool  RungoInput.Bind(&isok, "isok")  isok ==true
// var ft float64  RungoInput.Bind(&ft, "ft")  ft ==1.2
// ol := make([]int, 0, 2)  RungoInput.Bind(&ol, "ol")  ol ==[1 2]
// ul := make([]string, 0, 2)  RungoInput.Bind(&ul, "ul")  ul ==[str array]
// user struct{Name}  RungoInput.Bind(&user, "user")  user == {Name:"astaxie"}
func (input *RungoInput) Bind(dest interface{}, key string) error {
	value := reflect.ValueOf(dest)
	if value.Kind() != reflect.Ptr {
		return errors.New("rungo: non-pointer passed to Bind: " + key)
	}
	value = value.Elem()
	if !value.CanSet() {
		return errors.New("rungo: non-settable variable passed to Bind: " + key)
	}
	typ := value.Type()
	...
	if !rv.IsValid() {
		return errors.New("rungo: reflect value is empty")
	}
	value.Set(rv)
	return nil
}

func (input *RungoInput) bind(key string, typ reflect.Type) reflect.Value {
	if input.Context.Request.Form == nil {
		input.Context.Request.ParseForm()
	}
	rv := reflect.Zero(typ)
...
func (input *RungoInput) bindValue(val string, typ reflect.Type) reflect.Value {
	rv := reflect.Zero(typ)
	switch typ.Kind() {
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		rv = input.bindInt(val, typ)
...
func (input *RungoInput) bindInt(val string, typ reflect.Type) reflect.Value {
...
func (input *RungoInput) bindUint(val string, typ reflect.Type) reflect.Value {
...
func (input *RungoInput) bindFloat(val string, typ reflect.Type) reflect.Value {
...
func (input *RungoInput) bindString(val string, typ reflect.Type) reflect.Value {
	return reflect.ValueOf(val)
}

func (input *RungoInput) bindBool(val string, typ reflect.Type) reflect.Value {
...
func (input *RungoInput) bindSlice(params *url.Values, key string, typ reflect.Type) reflect.Value {
	maxIndex := -1
	numNoIndex := 0
...
func (input *RungoInput) bindStruct(params *url.Values, key string, typ reflect.Type) reflect.Value {
...
func (input *RungoInput) bindPoint(key string, typ reflect.Type) reflect.Value {
	return input.bind(key, typ.Elem()).Addr()
}

func (input *RungoInput) bindMap(params *url.Values, key string, typ reflect.Type) reflect.Value {
	var (
